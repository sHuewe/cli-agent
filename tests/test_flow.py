...
def test_overwrite_json_step_exposes_run_start_previous_output(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (tmp_path / "prompt.md").write_text(
        "state={{var:state}} count={{var:count}}",
        encoding="utf-8",
    )
    (tmp_path / "state.json").write_text(
        '{"count":1,"items":["a"]}',
...
""".strip(),
    ...
    assert [call.prompt for call in calls] == [
        'state={"count":1,"items":["a"]} count=1'
    ]


def test_previous_output_is_null_when_run_start_output_is_absent(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (tmp_path / "prompt.md").write_text(
...
""".strip(),
    ...
    async def fake_run_once(options, *, dependencies=None):
        calls.append(options)
        return SimpleNamespace(answer='{"count":1}', web_context_statuses=())
    ...
    assert calls[0].prompt == "state=null"
...
def test_previous_output_rejects_change_after_run_start(
    ...
    (tmp_path / "state.json").write_text('{"value":"old"}', encoding="utf-8")
    _write_config(tmp_path / "config.toml")
    (tmp_path / "flow.toml").write_text(
        """
version = 1
...
def test_chained_foreach_iterations_resume_recursively(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (tmp_path / "plan.md").write_text("plan", encoding="utf-8")
    (tmp_path / "analyze.md").write_text("analyze", encoding="utf-8")
    (tmp_path / "again.md").write_text("again", encoding="utf-8")
    (tmp_path / "analyze").mkdir()
    (tmp_path / "again").mkdir()
    (tmp_path / "plan.json").write_text(
        '{"items":[{"id":"one"},{"id":"two"}]}',
...
""".strip(),
    ...
    async def fake_run_once(options, *, dependencies=None):
        calls.append(options)
        raise AssertionError("all chained checkpoints must resume")
...
def test_chained_foreach_does_not_resume_from_incomplete_upstream_snapshot(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (tmp_path / "plan.md").write_text("plan", encoding="utf-8")
...
""".strip(),
    ...
    async def fake_run_once(options, *, dependencies=None):
        ...
        raise AssertionError("downstream existing file must collide, not resume")
