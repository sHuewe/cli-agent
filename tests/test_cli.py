...
from cli_agent.cli import apply_mcp_cli_overrides, apply_model_cli_override, _approval_arguments, build_parser
...
def test_approval_summary_hides_sensitive_tool_arguments() -> None:
    summary = _approval_arguments({"path": "src/main.py", "content": "confidential source", "auth-header": "Bearer confidential-token", "options": {"api_key": "nested-secret"}})
...
def test_with_os_write_replaces_configured_os_server() -> None:
    original = AppConfig(mcp_servers=(McpServerConfig(name="external", command="external-server"), McpServerConfig(name="os", command="legacy-os-server", config={"allow_write_files": False})))
...
def test_run_passes_network_policy_to_model_factory(tmp_path, monkeypatch) -> None:
    config = AppConfig(model=ModelConfig(provider="openai", model="internal-model", base_url="https://llm.internal/v1"), network=NetworkConfig(model_allowed_hosts=("llm.internal",)))
    captured = {}
    ...
    args = SimpleNamespace(config=None, model=None, os_access=None, debug=False, workspace=tmp_path, prompt=["hello"])
...
def test_model_cli_override_replaces_only_model_name() -> None:
    original = AppConfig(model=ModelConfig(provider="openai", model="configured-model", base_url="https://model-host.example/v1", api_key_env="MODEL_API_KEY", timeout=42.0, headers={"X-Test": "value"}))
