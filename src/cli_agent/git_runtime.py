...
class RuntimeGitWorkspace(GitWorkspace):
    ...
    def _repo(self, repository: str) -> GitRepository:
        ...
        for repo in self.repositories:
            ...
            candidate = (
                self.directory
                if normalized == "."
                else self.directory / normalized
            )
    ...
    def _validate_worktree_files(
        self, repo: GitRepository, path: str | None = None
    ) -> None:
        """Whole-repository validation is intentionally unsupported at runtime."""
        raise GitWorkspaceError(
            "Eine vollständige Working-Tree-Prüfung wird nicht unterstützt; "
            "Tools müssen relevante Dateien einzeln validieren."
        )

    def _validate_content_path(self, repo: GitRepository, relative: str) -> None:
        """Validate exactly one working-tree file before returning its contents."""
        ...
        try:
            ...
            for component in pure.parts:
                ...
                if path_entry_is_symlink_or_reparse(current):
                    raise GitWorkspaceError(
                        "Symlinks oder Reparse-Points in Working-Tree-Pfaden sind nicht erlaubt."
                    )
            status = os.lstat(entry)
            if not stat.S_ISREG(status.st_mode):
                raise GitWorkspaceError(
                    "Working-Tree-Inhalte müssen aus regulären Dateien stammen."
                )
            if status.st_nlink > 1:
                raise GitWorkspaceError(
                    "Working-Tree-Dateien mit mehreren Hardlinks werden aus Sicherheitsgründen nicht gelesen."
                )
            ...
            raise GitWorkspaceError(
                "Working-Tree-Datei konnte nicht sicher aufgelöst werden."
            ) from exc

    def _changed_worktree_paths(
        self, repo: GitRepository, path: str | None
    ) -> list[str]:
        args = [
            "diff",
            "--name-only",
    ...
    def git_grep(
        ...
        for file_index, filename in enumerate(matching_files):
            ...
            if remaining <= 0:
                truncated = True
                break
            # Filename discovery does not expose file contents. Validate this
            # exact file before the second grep can return matching source text.
            self._validate_content_path(repo, filename)
            output = self._git(
                repo.root,
                "grep",
