...
class CliAgent(McpLifecycleMixin, ConversationMixin):
    ...
    def _requires_approval(
        ...
        # User-provided MCP servers are not a security boundary. Their tool
        # metadata can claim "read-only" while the implementation performs
        # arbitrary side effects, so every call requires explicit approval.
        if not getattr(server_config, "built_in", False):
            return True
        if tool_name in WRITE_TOOLS:
            return bool(getattr(server_config, "allow_write_files", lambda: False)())
        if tool_name in COMPOSE_MUTATING_TOOLS:
            return bool(
                getattr(server_config, "allow_modify_services", lambda: False)()
            )
    ...
    async def _approve_tool_call(
        ...
        if self.approval_callback is None:
            logger.warning(
                "tool_call_rejected name=%s reason=approval_callback_missing",
                tool_name,
            )
            return False
        try:
            ...
            # Callback exceptions may contain user data or tool arguments;
            # keep the default log metadata-only.
            logger.error(
                "tool_call_approval_failed name=%s error_type=%s",
                tool_name,
                type(exc).__name__,
            )
            return False
