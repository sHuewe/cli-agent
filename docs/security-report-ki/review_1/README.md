...
3. **stdio-MCPs waren der auffälligste Bereich für weiteren Review.** Grok und Claude kritisierten die globale Startfreigabe über `allow_untrusted_stdio`; Claude und GPT-6 fanden unabhängig davon das Python-`-m`-Workspace-Shadowing. Beide konkreten stdio-Probleme sind inzwischen adressiert: `PYTHONSAFEPATH=1` schützt vor Workspace-Shadowing und jeder externe stdio-MCP benötigt nun ein eigenes administratives Launchprofil.
...
- **Granularität von `allow_untrusted_stdio`: inzwischen gelöst.** Der globale Schalter wurde entfernt. Externe stdio-MCPs können nur noch über einzeln administrativ definierte `[[mcp.trusted_servers]]`-Launchprofile gestartet werden; die Benutzerkonfiguration referenziert sie ausschließlich per Namen. Die Admin-Policy kontrolliert Command, Args und Env einschließlich des agentseitig aufgelösten `{workspace_directory}`.
- **MCP-Instructions:** externe, nicht explizit vertraute Instructions werden nicht mehr verworfen, sondern zusammen mit anderem untrusted Referenzkontext in einer transienten synthetischen `user`-Message vor der aktuellen echten User-Message bereitgestellt. Diese Message wird nicht in `self.history` persistiert. Nur identitätsgebundenes `trust_instructions = true` hebt Instructions weiterhin in den Systemprompt.
...
| Die globale Freigabe `allow_untrusted_stdio` erlaubte nach Aktivierung, dass normale Projektkonfiguration grundsätzlich beliebige lokale Programme als stdio-MCP startet. Der Schalter wurde entfernt. Jeder externe stdio-MCP benötigt nun ein konkretes administratives `[[mcp.trusted_servers]]`-Launchprofil; die Userconfig enthält für stdio nur den Namen und kann Command, Args oder Env nicht überschreiben. | Grok 4.6; Claude Opus 5 | `a463c3335a3088ed56933ba9b02a34446aeba9fd`, `fb07484fe48c43dd18141c83036d08db02b66230` |
| Externe MCP-Instructions wurden entweder privilegiert in den Systemprompt übernommen oder vollständig verworfen. Nicht explizit vertraute Instructions bleiben nun als transienter, klar als untrusted markierter Referenzkontext modell-sichtbar; derselbe Context-Block enthält optional OKF-, Web- und lokalen Datei-Kontext und wird nicht in der Conversation History gespeichert. `trust_instructions = true` bleibt als identitätsgebundene Ausnahme für administrativ kontrollierte Server bestehen. | Claude Opus 5 (F-02-Kontext) | `ce8e4e6e4fd976922555e1ac06a4c2f7aa21878f`, `97415e5c88fa9c825628b964c777eb45ef92be55` |
