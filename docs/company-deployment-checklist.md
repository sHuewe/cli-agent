...
- [x] **OKF-/Knowledge-Zugriffe innerhalb des gewählten Roots begrenzt.** Der in `[okf].repository` gewählte Root ist eine eigene read-only Dateisystemgrenze und darf bewusst außerhalb des Projekt-Workspaces liegen. Relative Escapes, absolute Folgepfade, Symlink-/Reparse-Escapes und Hardlink-Aliase werden abgefangen; Lese-/Indexlimits gelten. Der Agent akzeptiert nur vom Repository angebotene Folgepfade/-tools, begrenzt Tool-/Concept-Aufrufe und validiert die finale Token-Auswahl. **Die Auswahl des OKF-Roots selbst ist derzeit Benutzerkonfiguration und nicht durch eine Admin-Allowlist begrenzt.**
